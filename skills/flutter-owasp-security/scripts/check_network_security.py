...
def scan_http_usage(file_path: Path) -> List[Dict]:
    ...
    http_patterns = [
        ...
        (r'BaseOptions\s*\([^)]*baseUrl\s*:\s*["\']http://', 'Dio BaseOptions HTTP baseUrl'),
        # Retrofit @RestApi annotation
        (r'@RestApi\s*\(\s*baseUrl\s*:\s*["\']http://', 'Retrofit @RestApi HTTP baseUrl'),
    ]
...
def check_dio_log_interceptor(file_path: Path) -> List[Dict]:
    """Check for Dio LogInterceptor exposing headers/body without kDebugMode guard."""
    findings = []
    try:
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()

        # LogInterceptor with requestHeader or responseBody = true (not wrapped in kDebugMode)
        log_interceptor_match = re.search(
            r'LogInterceptor\s*\([^)]*(?:requestHeader|responseBody|requestBody)\s*:\s*true[^)]*\)',
            content,
        )
        if log_interceptor_match:
            # Check if it's guarded by kDebugMode in surrounding 3 lines
            start = max(0, log_interceptor_match.start() - 200)
            context = content[start:log_interceptor_match.end()]
            if 'kDebugMode' not in context:
                findings.append({
                    'file': str(file_path),
                    'line': content[:log_interceptor_match.start()].count('\n') + 1,
                    'issue': 'Dio LogInterceptor exposes headers/body without kDebugMode guard',
                    'code': log_interceptor_match.group(0)[:120],
                    'severity': 'HIGH',
                    'recommendation': 'Wrap LogInterceptor in `if (kDebugMode)` or set requestHeader/responseBody to false in production',
                })
    except Exception as e:
        print(f"Error scanning {file_path}: {e}")
    return findings
...
def scan_project(project_root: str) -> Dict:
    ...
    for file_path in dart_files:
        ...
        all_findings.extend(check_dio_log_interceptor(file_path))
